...
On Error Resume Next
   ...
   IP_SV_FATHER = "127.0.0.1" '"35.247.214.111" 'frmCargando.Inet1.OpenURL("https://marketingunlu.000webhostapp.com/IP2.txt")
   started = True
    
   mIP = "127.0.0.1" 'mdllUtil.IP_Publica 'agus: get my public IP
   sv.IP = mIP
   
   If Len(mIP) < 4 Then
      Call MsgBox("Error de conexion", vbCritical)
      End
   End If
 
   Call modManageUserMuertes.loadSave_Frag
    
   Set incomingData2 = New clsByteQueue
   Set outgoingData2 = New clsByteQueue

   frmMain.Winsock1.RemoteHost = IP_SV_FATHER
   frmMain.Winsock1.RemotePort = PORT_SV_FATHER
   ...
   DoEvents
    ...
    Call mod_indexHechiz.loadFastSpell
    ...
    Call modHandleData.writeCreateServer
    Call modHandleData.FlushBuffer2
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        Else
          ...
          If .Stats.MinHp < 1 Then 'repito esta linea xq estoy en modo debug y sino debo reiniciar. ACOMODAR!!
             Call definirPos(UserIndex)
             Call RevivirUsuario(UserIndex)
          End If
...
On Error GoTo ErrHandler
    ...
    For i = 1 To LastUser
        If UserList(i).flags.UserLogged Then
           ...
           If UserList(i).Counters.Equipar > 0 Then UserList(i).Counters.Equipar = UserList(i).Counters.Equipar - 1
           
           If UserList(i).flags.waitforConection > 0 Then UserList(i).flags.waitforConection = UserList(i).flags.waitforConection - 1
           
           If banning.count > 0 Then Call mod_ban.refreshListBan
...
Public Sub checkTrampa(ByVal UI As Integer)
With UserList(UI)

If MapData(.Pos.Map, .Pos.X, .Pos.Y).bombaInmo > 0 Then

   If MapData(.Pos.Map, .Pos.X, .Pos.Y).bombaInmo <> UI Then 'no podemos caer en nuestra propia trampa

      If Not .flags.Paralizado And Not .flags.Inmovilizado And Not .flags.Muerto Then
   
             .flags.Paralizado = 1
             .Counters.Paralisis = (IntervaloParalizado / 2)
             
             UserList(MapData(.Pos.Map, .Pos.X, .Pos.Y).bombaInmo).trap.Map = 0
             UserList(MapData(.Pos.Map, .Pos.X, .Pos.Y).bombaInmo).trap.X = 0
             UserList(MapData(.Pos.Map, .Pos.X, .Pos.Y).bombaInmo).trap.Y = 0
          
              Call SendData(SendTarget.toMap, .Pos.Map, PrepareMessageTrampa(.Pos.X, .Pos.Y, 1))
              Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg(.Name & " ha caído en una trampa de " & UserList(MapData(.Pos.Map, .Pos.X, .Pos.Y).bombaInmo).Name & "!!", FontTypeNames.FONTTYPE_FIGHT))
              Call SendData(SendTarget.ToPCArea, UI, PrepareMessageCreateFX(.Char.CharIndex, 1, 3))
              Call WriteParalizeOK(UI)
          
              MapData(.Pos.Map, .Pos.X, .Pos.Y).bombaInmo = 0
...
Public Sub insertarTrampa(ByVal UI As Integer, ByVal X As Byte, ByVal Y As Byte)
With UserList(UI)

     If .trap.Map > 0 Then
         Call WriteConsoleMsg(UI, "No puedes poner más de una trampa por mapa.", FontTypeNames.FONTTYPE_INFO)
         
     Else
     
     If .Stats.MaxMAN > 0 Then
     
        If (.Stats.MinMAN) < (.Stats.MaxMAN / 2) Then
           Call WriteConsoleMsg(UI, "Para lanzar este conjuro necesitas recargar más de la mitad de tu mana", FontTypeNames.FONTTYPE_INFO)
        Else
           If MapData(.Pos.Map, X, Y).bombaInmo > 0 Then
              Call WriteConsoleMsg(UI, "¡¡Cuidado!!, " & UserList(MapData(.Pos.Map, X, Y).bombaInmo).Name & " ya puso una trampa allí.", FontTypeNames.FONTTYPE_INFO)
           Else
              MapData(.Pos.Map, X, Y).bombaInmo = UI
              
              .trap.Map = .Pos.Map
              .trap.X = X
              .trap.Y = Y
              
              .Stats.MinMAN = .Stats.MinMAN - (.Stats.MaxMAN / 2)
              Call WriteUpdateUserStats(UI)
              Call SendData(SendTarget.toMap, .Pos.Map, PrepareMessageTrampa(X, Y, 0))
           End If
...
Public Sub deslogueaTrap(ByVal UI As Integer)
With UserList(UI)

     If .trap.Map > 0 Then
        MapData(.trap.Map, .trap.X, .trap.Y).bombaInmo = 0
        Call SendData(SendTarget.toMap, .Pos.Map, PrepareMessageTrampa(.trap.X, .trap.Y, 1))
     End If
