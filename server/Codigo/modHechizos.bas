...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        Call actualizarRanking(UserIndex, eModoRank.mkills)
...
With UserList(UserIndex)
    ...
    mapa = .Pos.Map
        ...
        If sv.carac.mascot < 1 Then
           Call WriteConsoleMsg(UserIndex, "¡Hechizo deshabilitado!", FontTypeNames.FONTTYPE_INFO)
        Exit Sub
        End If
...
'***************************************************
    ...
    Dim posCasteadaM As Integer
        ...
        If Hechizos(SpellIndex).trampa > 0 Then
        
          If sv.carac.trampa > 0 Then
        
           If MapData(posCasteadaM, posCasteadaX, posCasteadaY).UserIndex > 0 Then
              Call WriteConsoleMsg(UserIndex, "¡Te detienes porque algo lo impide!, allí hay un usuario parado.", FontTypeNames.FONTTYPE_INFO)
           ElseIf LegalPos(UserList(UserIndex).Pos.Map, posCasteadaX, posCasteadaY) = False Then
              Call WriteConsoleMsg(UserIndex, "¡Tu construcción se detiene porque esa posición es inválida!", FontTypeNames.FONTTYPE_CENTINELA)
           Else
              Call insertarTrampa(UserIndex, posCasteadaX, posCasteadaY)
           End If
...
With UserList(UserIndex)
    ...
    If Hechizos(HechizoIndex).Paraliza = 1 Or Hechizos(HechizoIndex).Inmoviliza = 1 Then
        ...
        If sv.carac.magoinmo Then
        
           If UserList(UserIndex).clase = eClass.Mage And _
              UserList(UserIndex).clase = UserList(TargetIndex).clase Then
              Call WriteConsoleMsg(UserIndex, "¡Hechizo deshabilitado para usar contra magos!", FontTypeNames.FONTTYPE_INFO)
           End If
        Exit Sub
        End If
...
With UserList(TargetIndex)
    ...
    If .Counters.Equipar > 0 Then
        Call WriteConsoleMsg(UserIndex, "¡Debes esperar " & .Counters.Equipar & " segundos aún!", FontTypeNames.FONTTYPE_INFO)
        Exit Function
    End If
...
If a > 0 Then actualizarRanking UserIndex, eModoRank.mkills
