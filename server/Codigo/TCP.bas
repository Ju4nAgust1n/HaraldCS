...
On Error GoTo ErrHandler
    With UserList(UserIndex)
        ...
        .flags.waitforConection = 0
        'Call deslogueaTrap(UserIndex)
        
        'If banning.index = UserIndex Then
         '  banning.count = 0
          ' banning.index = 0
          ' banning.lastCount = 0
          ' banning.vote = 0
        ...
        Call modHandleData.writeManageOnlines(1)
        
        'Call modHandleData.sendDataParch("REAINF" & NumUsers)
...
With UserList(UserIndex)
    ...
    If NumUsers >= sv.maxOn Then
        Call WriteErrorMsg(UserIndex, "El servidor ha alcanzado el máximo de usuarios, por favor vuelva a intertarlo más tarde.")
    ...
    For i = 1 To LastUser
       If UserList(i).ConnID <> -1 Then
        If UCase$(UserList(i).Name) = UCase$(Name) Then
              Call WriteErrorMsg(UserIndex, "Ya hay un usuario conectado con ese nombre.")
              Call FlushBuffer(UserIndex)
              Call CloseSocket(UserIndex)
           Exit Sub
        End If
   ...
   ' For i = 1 To LastUser
       ...
       '    If UCase$(UserList(i).Name) = UCase$(Name) Then
          '    If i <> UserIndex Then
           '      Call CloseSocket(i)
             '    Call WriteErrorMsg(i, "El servidor te ha deslogueado porque otra persona entró con tu personaje.")
            '  End If
           'End If
       ' End If
   ' Next i
    
    Call definirPos(UserIndex, 1)
    ...
    If UCase$(Name) = "AGUSH" Or UCase$(Name) = "THORKES" Then
    ...
    If Modalidad = eModalidad.Team Then
       modEvent.loguearTeam UserIndex
    Else
       .bando = 0
    End If
    ...
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("" & LCase$(.Name) & " ingresó.", FontTypeNames.FONTTYPE_TALK))
...
With UserList(UserIndex)
    ...
    If Modalidad = eModalidad.Team Then
       Call cerrarUserTeam(UserIndex)
    End If
    
    Call ResetUserSlot(UserIndex)
    
    Call closeUserRank(Name, bando)
