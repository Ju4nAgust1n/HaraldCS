...
On Error Resume Next
    ...
    Select Case packetID
        ...
        Case ClientPacketID.selectClass
            Call HandleSelectClass(UserIndex)
            
        Case ClientPacketID.banning
            Call HandleBanning(UserIndex)
            
        Case ClientPacketID.kicked
            Call HandleKicked(UserIndex)
...
#End If
        ...
        Else
            'Call ConnectUser(UserIndex, UserName, "") ' que haga el llamado el handledata
            writeReserveIndex UserIndex
        End If
#If SeguridadAlkon Then
    End If
#End If
...
On Error GoTo ErrHandler
    With UserList(UserIndex)
        ...
        If .flags.Privilegios >= PlayerType.SemiDios Then .flags.ChatColor = vbGreen
        
        If LenB(Chat) <> 0 Then
            'Analize chat...
            Call Statistics.ParseChat(Chat)
            
            If Chat <> " " And Chat <> "  " And Chat <> "-" Then
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If Modalidad = eModalidad.Team Then
           Call writeSndRank(UserIndex, eModoRank.mkills)
        Else
           Call writeSndRankDeath(UserIndex, eModoRank.mkills)
        End If
...
On Error GoTo ErrHandler
    Call UserList(UserIndex).outgoingData.WriteASCIIStringFixed(PrepareMessageCharacterCreate(body, Head, heading, CharIndex, X, Y, weapon, shield, FX, FXLoops, _
                                                            helmet, Name, NickColor, Privileges, Declaraciones.mIP))
Exit Sub
...
Public Function PrepareMessageCharacterCreate(ByVal body As Integer, ByVal Head As Integer, ByVal heading As eHeading, _
                                ByVal CharIndex As Integer, ByVal X As Byte, ByVal Y As Byte, ByVal weapon As Integer, ByVal shield As Integer, _
                                ByVal FX As Integer, ByVal FXLoops As Integer, ByVal helmet As Integer, ByVal Name As String, ByVal NickColor As Byte, _
                                ByVal Privileges As Byte, ByVal IP As String) As String
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
...
Public Sub HandleSelectClass(ByVal UserIndex As Integer)

With UserList(UserIndex)

    Dim index As Byte, raza As Byte, genero As Byte
    ...
    Call modClassSelect.selectClass(UserIndex, index, raza, genero)
    
End With

End Sub

Public Sub HandleBanning(ByVal UserIndex As Integer)
With UserList(UserIndex)
    ...
    Call mod_ban.makeListBan(UserIndex, Name)
    
End With

End Sub

Public Sub HandleKicked(ByVal UserIndex As Integer)
With UserList(UserIndex)

    Dim Name As String
    
    Call .incomingData.ReadByte
    Call mod_ban.voteBan(UserIndex)
...
Public Sub writeSndRank(ByVal UserIndex As Integer, ByVal mode As Byte)
On Error GoTo ErrHandler
Dim i As Long, dataCRI As String, dataCIU As String

Call UserList(UserIndex).outgoingData.WriteByte(ServerPacketID.Ranking)
    
         Select Case mode
         
                     Case eModoRank.mkills
                          dataCRI = prepareLstCri(mode)
                          dataCIU = prepareLstCiu(mode)
                          
                          Call UserList(UserIndex).outgoingData.WriteASCIIString(dataCRI)
                          Call UserList(UserIndex).outgoingData.WriteASCIIString(dataCIU)
...
Public Sub writeSndRankDeath(ByVal UserIndex As Integer, ByVal mode As Byte)
On Error GoTo ErrHandler
Dim i As Long, data As String

Call UserList(UserIndex).outgoingData.WriteByte(ServerPacketID.Ranking2)
    
         Select Case mode
         
                     Case eModoRank.mkills
                          data = prepareLstDeath(eModoRank.mkills)
                          Call UserList(UserIndex).outgoingData.WriteASCIIString(data)
...
Public Function PrepareMessageTrampa(ByVal X As Byte, ByVal Y As Byte, ByVal matarAnim As Byte) As String
    With auxiliarBuffer
        Call .WriteByte(ServerPacketID.trampa)
        Call .WriteByte(CByte(X))
        Call .WriteByte(CByte(Y))
        Call .WriteByte(CByte(matarAnim))
        
        PrepareMessageTrampa = .ReadASCIIStringFixed(.length)
    End With
End Function

Public Sub sendTrap(ByVal UserIndex As Integer, ByVal X As Byte, ByVal Y As Byte) 'para no hacer un bucle enorme che...
On Error GoTo ErrHandler
Dim i As Long, data As String

Call UserList(UserIndex).outgoingData.WriteByte(ServerPacketID.trampa)
Call UserList(UserIndex).outgoingData.WriteByte(X)
Call UserList(UserIndex).outgoingData.WriteByte(Y)
Call UserList(UserIndex).outgoingData.WriteByte(CByte(0))
...
Public Sub writeReserveIndex(ByVal UserIndex As Integer)
On Error GoTo ErrHandler
Dim i As Long, data As String

Call UserList(UserIndex).outgoingData.WriteByte(ServerPacketID.reserveIndex)
Call UserList(UserIndex).outgoingData.WriteInteger(UserIndex)
