...
On Error GoTo ErrHandler
    ...
    With UserList(AtacanteIndex)
       ...
       ' Call definirPos(VictimaIndex)
        Call RevivirUsuario(VictimaIndex)
    ...
    If a > 0 Then actualizarRanking AtacanteIndex, eModoRank.mkills
...
On Error GoTo ErrHandler
    ...
    If UserList(VictimIndex).Counters.Equipar > 0 Then
       Call WriteConsoleMsg(AttackerIndex, "No puedes atacarlo aún.", FontTypeNames.FONTTYPE_INFO)
       PuedeAtacar = False
       Exit Function
    End If
    
    If UserList(AttackerIndex).Counters.Equipar > 0 Then
       Call WriteConsoleMsg(AttackerIndex, "Debes esperar " & UserList(AttackerIndex).Counters.Equipar & " segundos aún.", FontTypeNames.FONTTYPE_INFO)
       PuedeAtacar = False
       Exit Function
    End If
    
    If Modalidad = eModalidad.Team Then
       If UserList(AttackerIndex).bando > 0 Then
          If UserList(AttackerIndex).bando = UserList(VictimIndex).bando Then
             Call WriteConsoleMsg(AttackerIndex, "¡Es tu compañero!", FontTypeNames.FONTTYPE_INFO)
             PuedeAtacar = False
             Exit Function
          End If
       End If
