...
#End If
        ...
        If UserList(NewIndex).IP = IP_SV_FATHER And started Then
                started = False
        ...
        If UserList(NewIndex).flags.waitforConection > 0 Or UserList(NewIndex).ConnID > -1 Then
                Call WriteErrorMsg(NewIndex, "Aguarde unos instantes...")
                Call FlushBuffer(NewIndex)
                Call SecurityIp.IpRestarConexion(sa.sin_addr)
                Call WSApiCloseSocket(NuevoSock)
        Exit Sub
        ...
        UserList(NewIndex).flags.waitforConection = 5
