...
On Error Resume Next
    ...
    Select Case Packet
       ...
       Case ServerPacketID.Ranking2
            Call HandleRanking2
            
       Case ServerPacketID.trampa
            Call HandleTrampView
            
       Case ServerPacketID.reserveIndex
            Call handleReserveIndex 'ahora logueamos por acá
...
Public Sub HandleTrampView()
With outgoingData
     Call incomingData.ReadByte
     
Dim X As Byte, Y As Byte, matarAnim As Byte

X = incomingData.ReadByte()
Y = incomingData.ReadByte()
matarAnim = incomingData.ReadByte()

If matarAnim = 0 Then
   MapData(X, Y).trampa = 1
   Call InitGrh(grhTrampa, FxData(grhTrampa.GrhIndex).Animacion)
Else
   MapData(X, Y).trampa = 0
End If

End With
End Sub

Public Sub handleReserveIndex()
With outgoingData
Dim UI As Integer

Call incomingData.ReadByte
UI = incomingData.ReadInteger()

If (UI > 0) Then
   Call mdllProtocol.writeLoginCharacter(UI)
   Call FlushBuffer2
End If
...
'***************************************************
    ...
    If UserMaxMAN > 0 Then _
        bwidth = (((UserMinMAN / 100) / (UserMaxMAN / 100)) * 95)
        
    frmMain.shpMana.Width = 95 - bwidth
    frmMain.shpMana.Left = 560 + (95 - frmMain.shpMana.Width)
    
    frmMain.shpMana.Visible = (bwidth <> 95)
End Sub
...
'***************************************************
    ...
    bwidth = (((UserMinHP / 100) / (UserMaxHP / 100)) * 95)
    
    frmMain.shpVida.Width = 95 - bwidth
    frmMain.shpVida.Left = 560 + (95 - frmMain.shpVida.Width)
    
    frmMain.shpVida.Visible = (bwidth <> 95)
...
On Error GoTo ErrHandler
    ...
    If NickColor = 100 Then
            .death = 2
    ElseIf NickColor = 101 Then
            .death = 3
    End If
    ...
    IP = Buffer.ReadASCIIString()
    
    If (miIP = IP) Then charlist(CharIndex).creador = True
...
Dim cri As String, ciu As String, i As Long
    
    Call incomingData.ReadByte
    
    cri = incomingData.ReadASCIIString
    ciu = incomingData.ReadASCIIString

    Dim count As Byte, count2 As Byte
        count = CByte(ReadField(1, cri, Asc(",")))
        count2 = 1
        
     For i = 1 To count
         count2 = count2 + 1
         crimis.user(i) = ReadField(count2, cri, Asc(","))
         count2 = count2 + 1
         crimis.muertes(i) = ReadField(count2, cri, Asc(","))
         
         If Len(crimis.user(i)) > 0 Then frmRank.lstCRI.AddItem i & "º " & crimis.user(i) & " <Kills: " & crimis.muertes(i) & ">"
         
     Next i
     
         count = CByte(ReadField(1, ciu, Asc(",")))
         count2 = 1
     
     For i = 1 To count
         count2 = count2 + 1
         ciudas.user(i) = ReadField(count2, ciu, Asc(","))
         count2 = count2 + 1
         ciudas.muertes(i) = ReadField(count2, ciu, Asc(","))
         
         If Len(ciudas.user(i)) > 0 Then frmRank.lstCIU.AddItem i & "º " & ciudas.user(i) & " <Kills: " & ciudas.muertes(i) & ">"
         
     Next i
     
     frmRank.Show , frmMain

End Sub

Private Sub HandleRanking2()
Dim cri As String, ciu As String, i As Long
    ...
    Dim count As Byte, count2 As Byte
     ...
     For i = 1 To count
         count2 = count2 + 1
         death.user(i) = ReadField(count2, cri, Asc(","))
         count2 = count2 + 1
         death.muertes(i) = ReadField(count2, cri, Asc(","))
         
         If Len(death.user(i)) > 0 Then frmRankDeath.lstDeath.AddItem i & "º " & death.user(i) & " <Kills: " & death.muertes(i) & ">"

     Next i
     
     frmRankDeath.Show , frmMain
...
If frmRank.Visible Or frmRankDeath.Visible Then Exit Sub
...
Public Sub writeselectClass(ByVal Index As Byte, ByVal Razas As Byte, ByVal Genero As Byte)
    With outgoingData
        Call .WriteByte(ClientPacketID.selectClass)
        Call .WriteByte(Index)
        Call .WriteByte(Razas)
        Call .WriteByte(Genero)
    End With
End Sub

Public Sub writeBanning(ByVal name As String)
    With outgoingData
        Call .WriteByte(ClientPacketID.banning)
        Call .WriteASCIIString(name)
    End With
End Sub

Public Sub writeKicked()
    With outgoingData
        Call .WriteByte(ClientPacketID.kicked)
    End With
End Sub
