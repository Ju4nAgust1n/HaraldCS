...
Public tempWidth As Single

Public Sub addWidthVar(Optional ByVal last As Byte = 0)

With frmCargando

If (last > 0) Then
   .Picture1.Width = tempWidth
Else
Debug.Print .Picture1.Width
   .Picture1.Width = .Picture1.Width + (tempWidth * 0.05)
End If
...
#End If
    ...
    Call ShowConsoleMsg("¡¡Bienvenido a HaraldAO, el mejor server de la historia ;-). Versión ALPHA TEST!!", 255, 255, 255)
    Call AddtoRichTextBox(frmMain.RecTxt, "Recuerda que con la tecla ESC puedes visualizar el RANKING de personajes.", 255, 0, 0, True, False)
...
#End If
    ...
    frmMain.shpHambre.Visible = False
    frmMain.shpSed.Visible = False
    frmMain.shpEnergia.Visible = False
   ...
   frmConnect.Winsock1.RemoteHost = IP_SV_FATHER
   frmConnect.Winsock1.RemotePort = PORT_SV_FATHER
   frmConnect.Winsock1.Close
   frmConnect.Winsock1.Connect
...
#End If
         ...
         If canRun < 1 And frmMain.Visible And Not frmClass.Visible Then
            frmClass.Show , frmMain
            Call MsgBox("¡No puedes cerrar el panel sin definir los atributos de tu personaje!", vbInformation, "Harald")
         End If
...
'***************************************************
    ...
    IP_SV_FATHER = "35.247.214.111" 'frmCargando.Inet1.OpenURL("https://marketingunlu.000webhostapp.com/IP2.txt")
    
    Call CargarServidores
    ServersRecibidos = True
    
    If Len(IP_SV_FATHER) > 3 Then
       Call AddtoRichTextBox(frmCargando.Status, "Cargado correctamente!", 0, 255, 0, True, False, False)
    Else
       Call AddtoRichTextBox(frmCargando.Status, "¡Error al cargar!", 255, 0, 0, True, False, False)
    End If
...
#End If
    ...
    If Not InitTileEngine(frmMain.hWnd, frmMain.MainViewShp.Top, frmMain.MainViewShp.Left, 32, 32, 13, 17, 9, 8, 8, 0.018) Then
        Call CloseClient
    End If
    Call AddtoRichTextBox(frmCargando.Status, "Hecho", 255, 0, 0, True, False, False)
...
'**************************************************************
    ...
    Set incomingData2 = Nothing
    Set outgoingData2 = Nothing
...
If Left(Text, Len(MENSAJE_FRAGSHOOTER_HAS_MATADO)) = MENSAJE_FRAGSHOOTER_HAS_MATADO Then
    Call mdllProtocol.writesendKills
    Exit Sub
End If
